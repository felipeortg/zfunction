...
lab_energy = 10.0
cm_energy = np.sqrt(np.square(lab_energy) - lab_moment.dot(lab_moment))
...
d_list = []
for x in xrange(0,n_arr.shape[0]): d_list.append(d)
...
w_par_arr = np.transpose(np.append([d_w_dot],[d_w_dot,d_w_dot],axis=0)) * d_arr
